#!/usr/bin/env python3
"""Run a fixture-backed write/restore recovery drill for cognitive scripts."""
...
ROOT = Path.home()
SOURCE_SCRIPT = ROOT / "cognitive" / "appraisal.py"
WORK_ROOT = ROOT / "tmp" / "recovery-drills" / "cognitive-scripts"
OUTPUT_JSON = ROOT / "data" / "cognitive-scripts-recovery-drill" / "latest.json"
OUTPUT_HISTORY = ROOT / "data" / "cognitive-scripts-recovery-drill" / "history.jsonl"
OUTPUT_MD = ROOT / "context" / "cognitive-scripts-recovery-drill.md"
...
def run_drill() -> dict[str, Any]:
    ...
    fixture = run_dir / SOURCE_SCRIPT.name
    backup = run_dir / f"{SOURCE_SCRIPT.name}.restore"
    shutil.copy2(SOURCE_SCRIPT, fixture)
    shutil.copy2(fixture, backup)

    source_hash = sha256(SOURCE_SCRIPT)
    before_hash = sha256(fixture)

    with fixture.open("a", encoding="utf-8") as handle:
        handle.write(f"\n# cognitive scripts recovery drill mutation {generated_at}\n")
    after_hash = sha256(fixture)

    shutil.copy2(backup, fixture)
    restore_hash = sha256(fixture)
    backup_hash = sha256(backup)
    live_hash_after = sha256(SOURCE_SCRIPT)
    ...
    return {
        "schema": "seed.cognitive_scripts_recovery_drill.v1",
        "generated_at": generated_at,
        "cycle": cycle,
        "surface": "cognitive_scripts",
        "fixture_source": rel(SOURCE_SCRIPT),
        ...
        "checks": {
            ...
            "live_script_unchanged": live_hash_after == source_hash,
        },
        "recovery_proof": (
            "A disposable copy of a live cognitive script was mutated, restored from a local restore source, "
            "and verified by before_hash/after_hash/restore_hash without changing the live script."
        ),
        "next": "Let supply_chain_prober consume this as explicit cognitive_scripts recovery drill evidence.",
    }


def render_markdown(receipt: dict[str, Any]) -> str:
    lines = [
        f"# Cognitive Scripts Recovery Drill - {receipt['generated_at']}",
        "",
        "Fixture-backed recovery drill for the `cognitive_scripts` surface.",
        "",
